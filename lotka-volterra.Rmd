...
library(tidyverse)
library(deSolve)
library(kableExtra) ##camelcase is when uppercase is in the middle
```
...
time <- seq(0, 25, by = 0.05) ## this would be our approximations we would do by hand, which would be a ton!
...
ggplot(data = approx_lk_df, aes(x = time, y = population)) +
  geom_line(aes(color = species)) ##Looks continuous because our approximations are so close together
```
